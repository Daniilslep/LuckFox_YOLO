...
Если в `/etc/luckfox.cfg` уже есть `I2C3_M1_STATUS=1`, но `/dev/i2c-3` нет — выполните ещё раз:
...
На части образов (особенно Mini B со встроенной Flash) overlay поднимается именно командой `load`, одного редактирования файла иногда недостаточно. После перезагрузки платы при необходимости снова выполните `luckfox-config load`.

> Настройка `/etc/luckfox.cfg` обычно сохраняется между перезагрузками. Если файл «сжался» до пары строк или вы перепрошивали плату — шаги выше нужно повторить.
