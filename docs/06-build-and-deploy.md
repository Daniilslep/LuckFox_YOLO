...
К этому моменту у вас должны быть два файла с предыдущего шага: `yolov8.rknn` и `labels.txt`.
...
1. Откройте новый ноутбук на https://colab.research.google.com/ (GPU для этого шага не нужен, можно оставить среду по умолчанию).
2. Загрузите в сессию Colab (иконка «Файлы» слева → «Загрузить в сессионное хранилище») два файла с прошлого шага: `yolov8.rknn` и `labels.txt`.
3. Откройте файл [`colab/02_build_hellodetector.py`](../colab/02_build_hellodetector.py) из этого репозитория, скопируйте всё его содержимое в одну ячейку Colab.
4. В начале скрипта поправьте пути `MODEL_RKNN_PATH` и `LABELS_TXT_PATH`, если назвали файлы иначе или загрузили не в `/content/`.
5. Запустите ячейку. Скрипт сам:
   - скачает кросс-компилятор Luckfox;
   - склонирует базовый проект [ret7020/Yolov8CustomNPU](https://github.com/ret7020/Yolov8CustomNPU) (в нём уже есть обёртки над RKNN и нужные библиотеки под плату);
   - склонирует этот репозиторий и подставит универсальный `main.cc` с поддержкой OLED и светодиода;
   - подставит вашу модель и `labels.txt`, автоматически поправит число классов в коде;
   - обновит OpenCV Mobile до версии 4.10 (на части прошивок старая версия из базового проекта не совместима с камерой — версия 4.10 работает надёжно);
   - соберёт проект и скачает готовый архив `HelloDetector_bin.zip`.

Если что-то пошло не так — читайте вывод ячейки, там печатаются все шаги и ошибки cmake/make, если они возникли.
